...
let fetchPeopleWithPromises = () => {
  return superagent
    ...
    .then(peoplePromises => {
      return Promise.all(peoplePromises).then(people => {
        let names = [];
...
let fetchPeopleWithAsync = async () => {
  try {
    ...
    let swapiNames = await Promise.all(peopleRequests).then(people => {
      let names = [];
      for (let data of people) {
        names.push(data.body.name);
      }
    ...
    console.error('Michael messed up', error);
  }
};
fetchPeopleWithPromises().then(people => {
  console.log('Promise: ', people);
});
fetchPeopleWithAsync().then(names => {
  console.log('Async: ', names);
});
